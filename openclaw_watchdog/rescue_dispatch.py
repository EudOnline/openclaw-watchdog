...
from openclaw_watchdog.rescue_models import RescueAttempt, RescueContext, RescuePlan
    ...
    def dispatch(self, context: RescueContext) -> DispatchResult:
        ...
        for adapter in self.adapters:
            ...
            if plan is None:
                ...
                continue
            try:
                rescue_policy.validate_plan(context, plan)
            except Exception as exc:
                result.attempts.append(RescueAttempt(executor=adapter.name, status='unsafe-plan', error=str(exc)))
                continue
