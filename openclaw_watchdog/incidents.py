...
def incident_snapshot(engine, incident_dir: Path) -> dict[str, object]:
    ...
    state_payload = incident_service_ops.read_incident_state_payload(engine, incident_dir)
    operator_payload = incident_service_ops.read_incident_operator_summary_payload(engine, incident_dir)
    workflow_payload = incident_service_ops.read_incident_operator_workflow_payload(engine, incident_dir)
    index_payload = next(
        (
            item
            for item in reversed(incident_service_ops.read_incident_index(engine))
    ...
    snapshot = {
        ...
        "rollback_occurred": incident_service_ops.incident_bool(index_payload.get("rollback_occurred", operator_payload.get("rollback_occurred", False))),
        ...
        "acknowledged": incident_service_ops.incident_bool(workflow_payload.get("acknowledged", index_payload.get("acknowledged", False))),
...
def update_incident_operator_workflow(
    ...
    *,
    ...
    if not incident_dir.exists() or not incident_dir.is_dir():
        return {}
    payload = incident_service_ops.read_incident_operator_workflow_payload(engine, incident_dir)
    ...
    incident_service_ops.incident_operator_workflow_file(engine, incident_dir).write_text(
