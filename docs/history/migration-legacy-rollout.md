...
- The new implementation lives in `openclaw_watchdog/` and is invoked by `scripts/openclaw-watchdog`.
...
- `openclaw_watchdog/config.py`: env loading and defaults.
- `openclaw_watchdog/engine.py`: orchestration shell that wires together health checks, repair flow, rollback, incidents, reporting, and AI handoff.
- `openclaw_watchdog/incidents.py`: incident snapshot/index/workflow helpers used by the engine.
- `openclaw_watchdog/reporting.py`: report/message/metrics rendering and snapshot export helpers used by the engine.
- `openclaw_watchdog/repair.py`: repair/rollback helpers (doctor invocation, last-good backup/restore, rollback summaries, pre-repair backup, service restart helpers) delegated from the engine.
- `openclaw_watchdog/survival.py`: survival-mode state/config generation and sticky degraded-mode coordination.
- `openclaw_watchdog/health.py`: service-level probing, live probe aggregation, maintenance/status payload assembly, and last-event loading delegated from the engine.
- `openclaw_watchdog/handoff.py`: incident evidence bundle creation plus Codex/OpenCode handoff prompt/launcher helpers delegated from the engine.
...
- `openclaw_watchdog/cli.py`: operator-facing CLI.
- `openclaw_watchdog/bootstrap.py`: OpenClaw/bootstrap provisioning helpers.
...
- `WATCHDOG_OPENCODE_BIN`: runtime OpenCode binary used by the watchdog fallback path. On this host we keep it pinned to `/root/.opencode/bin/opencode` for reliability; on other hosts you can switch it to a PATH-resolved `opencode` if that install path is stable.
