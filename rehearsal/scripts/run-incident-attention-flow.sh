...
LIST_YES=$(scripts/openclaw-watchdog-v2 --env "$ENV_FILE" incidents list --json --attention yes --limit 5)
SHOW_OPEN=$(scripts/openclaw-watchdog-v2 --env "$ENV_FILE" incidents show "$INCIDENT_ID" --json)

scripts/openclaw-watchdog-v2 --env "$ENV_FILE" incidents assign "$INCIDENT_ID" --owner alice >/dev/null
scripts/openclaw-watchdog-v2 --env "$ENV_FILE" incidents ack "$INCIDENT_ID" --by alice --note "triaging attention queue" >/dev/null

LIST_NO=$(scripts/openclaw-watchdog-v2 --env "$ENV_FILE" incidents list --json --attention no --limit 5)
CURRENT_JSON=$(scripts/openclaw-watchdog-v2 --env "$ENV_FILE" incidents current --json)
