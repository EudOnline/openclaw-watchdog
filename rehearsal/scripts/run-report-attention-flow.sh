...
case "$MODE" in
  open-unowned)
    ...
    set -e
    scripts/openclaw-watchdog --env "$ENV_FILE" report --json --limit 3
    ;;
  open-owned)
    set +e
    scripts/openclaw-watchdog --env "$ENV_FILE" run-once --json >/dev/null
    ...
    scripts/openclaw-watchdog --env "$ENV_FILE" incidents ack "$INCIDENT_ID" --by alice --note "investigating attention clear" >/dev/null
    scripts/openclaw-watchdog --env "$ENV_FILE" report --json --limit 3
    ;;
  *)
