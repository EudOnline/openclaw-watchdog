...
scripts/openclaw-watchdog-v2 --env "$ENV_FILE" incidents unassign "$INCIDENT_ID" >/dev/null
scripts/openclaw-watchdog-v2 --env "$ENV_FILE" incidents unack "$INCIDENT_ID" >/dev/null

LIST_ALL=$(scripts/openclaw-watchdog-v2 --env "$ENV_FILE" incidents list --json --limit 5)
LIST_OWNER_ALICE=$(scripts/openclaw-watchdog-v2 --env "$ENV_FILE" incidents list --json --owner alice --limit 5)
LIST_ACK_YES=$(scripts/openclaw-watchdog-v2 --env "$ENV_FILE" incidents list --json --ack yes --limit 5)
LIST_ACK_NO=$(scripts/openclaw-watchdog-v2 --env "$ENV_FILE" incidents list --json --ack no --limit 5)
