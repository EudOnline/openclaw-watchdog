...
case "$MODE" in
  healthy)
    ...
    scripts/openclaw-watchdog --env "$ENV_FILE" metrics --json
    ;;
  open)
    set +e
    scripts/openclaw-watchdog --env "$ENV_FILE" run-once --json >/dev/null
...
PY
    ...
    scripts/openclaw-watchdog --env "$ENV_FILE" incidents ack "$INCIDENT_ID" --by alice --note "investigating exporter view" >/dev/null
    scripts/openclaw-watchdog --env "$ENV_FILE" incidents note "$INCIDENT_ID" --by bob --message "collector should see operator context" >/dev/null
    scripts/openclaw-watchdog --env "$ENV_FILE" metrics --json
    ;;
  *)
