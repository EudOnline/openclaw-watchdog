...
def run(engine, ctx):
    ...
    engine.last_good_validated_at = str(last_good.get("last_good_validated_at", "") or "")
    engine.last_good_generation_id = str(last_good.get("last_good_generation_id", "") or "")
    engine.last_good_generation_count = int(last_good.get("last_good_generation_count", 0) or 0)

    try:
        probe = engine.live_probe(include_doctor=True, apply_grace=True)
        engine.latest_probe = dict(probe)
        ...
        if process_layer_healthy and conversation_ready and not config_invalid:
            ...
            if not engine.survival_mode_active:
        ...
        engine.config_drift_detected = bool(drift.get("detected", False))
        engine.drift_scope = [str(item) for item in drift.get("scope", []) if str(item).strip()]
        engine.drift_since_last_good = str(drift.get("since_last_good", "") or "")
        engine.drift_summary = str(drift.get("summary", "") or "")
        ...
        if config_invalid:
            ...
            if restart_ok:
                ...
                engine.latest_probe = dict(restart_probe)
        ...
        if engine.config.watchdog_last_good_config.exists() or engine.config.watchdog_last_good_manifest_file.exists():
            rollback_reason = (
                ...
                if engine.config_drift_detected
            ...
            engine.record_recovery_step(
                ...
                engine.rollback_candidate_used or rollback_reason,
            )
            if rollback_ok:
                ...
                if rollback_restart_ok:
                    ...
                    engine.latest_probe = dict(rollback_probe)
                    ...
                    if bool(rollback_probe.get("minimal_usable_ready", False)):
                        ...
                        summary = (
                            f"rollback restored conversation readiness (candidate={engine.rollback_candidate_used or 'unknown'})"
                            if bool(rollback_probe.get("conversation_ready", False))
                            else f"rollback restored minimal usable conversation (candidate={engine.rollback_candidate_used or 'unknown'})"
                        )
                        engine.set_state("recovered", summary)
        ...
        if bool(survival_result.get("applied", False)):
            ...
            if survival_restart_ok:
                ...
                engine.latest_probe = dict(survival_probe)
        ...
        if engine.config.watchdog_enable_doctor_repair:
            ...
            if doctor_restart_ok:
                ...
                engine.latest_probe = dict(doctor_probe)
        ...
        final_probe = dict(engine.latest_probe) if engine.latest_probe else engine.live_probe(include_doctor=False, apply_grace=True)
        ...
        engine.write_run_state(
            {
                ...
                "health_level": "failed",
                "current_mode": engine.current_mode(maintenance=engine.config.watchdog_maintenance_file.exists(), survival=engine.survival_mode_active),
                ...
                "last_recovery_strategy": engine.last_recovery_strategy,
                "last_recovery_path": engine.recovery_path_text(),
                "last_recovery_action_count": engine.last_recovery_action_count,
                "last_recovery_restored_conversation": engine.last_recovery_restored_conversation,
                "rollback_candidate_used": engine.rollback_candidate_used,
                "rollback_reason": engine.rollback_reason,
                "config_drift_detected": engine.config_drift_detected,
                "drift_scope": list(engine.drift_scope),
                "drift_since_last_good": engine.drift_since_last_good,
                "drift_summary": engine.drift_summary,
