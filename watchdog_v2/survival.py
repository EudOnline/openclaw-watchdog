...
def _apply_state_to_engine(engine, state: dict[str, object]) -> None:
    engine.survival_mode_active = bool(state.get("active", False))
    engine.survival_mode_reason = str(state.get("reason", "") or "") if engine.survival_mode_active else ""
    engine.survival_mode_since = str(state.get("entered_at", "") or "") if engine.survival_mode_active else ""
    engine.survival_mode_summary = str(state.get("summary", "") or "") if engine.survival_mode_active else ""
    engine.survival_mode_actions = [str(item) for item in state.get("actions", []) if str(item).strip()] if engine.survival_mode_active else []
    engine.survival_mode_disabled_features = [
        str(item) for item in state.get("disabled_features", []) if str(item).strip()
    ] if engine.survival_mode_active else []
    engine.survival_mode_config_file = (
        str(state.get("config_path", engine.config.watchdog_survival_config_file) or engine.config.watchdog_survival_config_file)
        if engine.survival_mode_active
        else ""
    )
    engine.survival_mode_sticky = bool(state.get("sticky", False)) if engine.survival_mode_active else False
    engine.survival_mode_sticky_reason = str(state.get("sticky_reason", "") or "") if engine.survival_mode_active else ""
    engine.survival_mode_exit_ready = bool(state.get("exit_ready", False)) if engine.survival_mode_active else False
    engine.survival_mode_exit_policy = str(state.get("exit_policy", "") or "") if engine.survival_mode_active else "none"
    engine.survival_mode_exit_blockers = [
        str(item) for item in state.get("exit_blockers", []) if str(item).strip()
    ] if engine.survival_mode_active else []
    engine.survival_mode_stable_ready_runs = int(state.get("stable_ready_runs", 0) or 0) if engine.survival_mode_active else 0
    engine.survival_mode_stable_required_runs = int(state.get("stable_required_runs", _stable_required_runs(engine)) or _stable_required_runs(engine))
    engine.survival_mode_manual_clear_required = bool(state.get("manual_clear_required", False)) if engine.survival_mode_active else False
    engine.survival_mode_config_changed_away = bool(state.get("config_changed_away", False)) if engine.survival_mode_active else False
    engine.survival_mode_last_exit_at = str(state.get("last_exit_at", "") or "")
    engine.survival_mode_last_exit_reason = str(state.get("last_exit_reason", "") or "")
    engine.survival_mode_last_exit_kind = str(state.get("last_exit_kind", "") or "")
    engine.survival_mode_last_exit_summary = str(state.get("last_exit_summary", "") or "")


def run_state_fields(engine) -> dict[str, object]:
    return {
        "survival_mode_active": engine.survival_mode_active,
        "survival_mode_reason": engine.survival_mode_reason,
        "survival_mode_since": engine.survival_mode_since,
        "survival_mode_summary": engine.survival_mode_summary,
        "survival_mode_actions": list(engine.survival_mode_actions),
        "survival_mode_disabled_features": list(engine.survival_mode_disabled_features),
        "survival_mode_config_file": engine.survival_mode_config_file,
        "survival_mode_sticky": engine.survival_mode_sticky,
        "survival_mode_sticky_reason": engine.survival_mode_sticky_reason,
        "survival_mode_exit_ready": engine.survival_mode_exit_ready,
        "survival_mode_exit_policy": engine.survival_mode_exit_policy,
        "survival_mode_exit_blockers": list(engine.survival_mode_exit_blockers),
        "survival_mode_stable_ready_runs": engine.survival_mode_stable_ready_runs,
        "survival_mode_stable_required_runs": engine.survival_mode_stable_required_runs,
        "survival_mode_manual_clear_required": engine.survival_mode_manual_clear_required,
        "survival_mode_config_changed_away": engine.survival_mode_config_changed_away,
        "survival_mode_last_exit_at": engine.survival_mode_last_exit_at,
        "survival_mode_last_exit_reason": engine.survival_mode_last_exit_reason,
        "survival_mode_last_exit_kind": engine.survival_mode_last_exit_kind,
        "survival_mode_last_exit_summary": engine.survival_mode_last_exit_summary,
    }
