...
def render_report(payload: dict[str, object]) -> str:
    ...
    lines = [
        ...
        f"rescue_attempt_count={payload.get('rescue_attempt_count', 0)}",
        f"rescue_executor_selected={payload.get('rescue_executor_selected', '') or 'none'}",
        f"rescue_plan_generated={str(bool(payload.get('rescue_plan_generated', False))).lower()}",
        f"rescue_plan_source={payload.get('rescue_plan_source', '') or 'none'}",
        f"rescue_plan_status={payload.get('rescue_plan_status', '') or 'none'}",
        f"rescue_tier={payload.get('rescue_tier', '') or 'none'}",
        f"case_ingest_result={payload.get('case_ingest_result', '') or 'none'}",
        f"candidate_rule_status={payload.get('candidate_rule_status', '') or 'none'}",
        f"rescue_attempt_order={_list_value(payload, 'rescue_attempt_order', joiner=' -> ')}",
        f"rescue_rejected_executors={_list_value(payload, 'rescue_rejected_executors', joiner=', ')}",
        f"rescue_learning_summary={payload.get('rescue_learning_summary', '') or 'none'}",
        f"rescue_mutation_scope={_list_value(payload, 'rescue_mutation_scope', joiner=', ')}",
